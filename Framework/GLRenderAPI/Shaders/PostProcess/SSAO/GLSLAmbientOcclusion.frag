...
void main()
{
    vec3 position   = vec3(View * (texture(gPosition,    fs_in.FragTexCoords).xyz, 1.0));
    vec3 normal     = vec3(View * (texture(gNormal,      fs_in.FragTexCoords).xyz, 0.0));
    ...
    for (uint i = 0; i < samplesCount; ++i)
    {
        ...
        vec4 pos = View * vec4(texture(gPosition, offset.xy).xyz, 1.0);
        float sampleDepth = pos.z;
        //float sampleDepth = texture(gPosition, offset.xy).z;

        //float rangeCheck = smoothstep(0.0, 1.0, radius / abs(position.z - sampleDepth));
        //occlusion += (sampleDepth < current.z + bias ? 1.0 : 0.0) * rangeCheck;
        occlusion += (sampleDepth < current.z - bias ? 1.0 : 0.0);
    }

    mat4 f = PV;
	//AmbientOcclusion = occlusion / float(samplesCount);
    AmbientOcclusion = vec3(View * vec4(texture(gPosition,    fs_in.FragTexCoords).xyz, 1.0)).z * 0.05 + 1.0;
}
